...
static void nic_dma_callback_tx(void *data)
{
    ...
    if (!txr) {
        pr_err("nic_dma_callback_tx: NULL tx_request\n");
        return;
    }
    ...
    if (!nic) {
        pr_err("nic_dma_callback_tx: tx_request->cb_data NULL\n");
        return;
    }

    if (!nic->dma_tx_chan.chan) {
        pr_err("nic_dma_callback_tx: dma_tx_chan.chan NULL\n");
        return;
    }
...
static void nic_dma_callback_rx(void *data)
{
    ...
    if (!rxr) {
        pr_err("nic_dma_callback_rx: NULL rx_request\n");
        return;
    }
    ...
    if (!nic) {
        pr_err("nic_dma_callback_rx: rx_request->cb_data NULL\n");
        return;
    }

    if (!nic->dma_rx_chan.chan) {
        pr_err("nic_dma_callback_rx: dma_rx_chan.chan NULL\n");
        return;
    }

    enum dma_status status;
    struct dma_tx_state state;
    status = dmaengine_tx_status(nic->dma_rx_chan.chan, rxr->cookie, &state);

    if (status != DMA_COMPLETE) {
        pr_err("DMA CALLBACK STATUS ERROR\n");
        rxr->data_len = 0;
    } else {
        if (state.residue <= rxr->data_len)
            rxr->data_len -= state.residue;
        else
            rxr->data_len = 0;
    }
