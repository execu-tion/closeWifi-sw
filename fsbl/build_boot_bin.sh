...
# deal with some variables
XSA_FILE_NAME=`basename $XSA_FILE`
BIT_FILE_NAME=`basename -s .xsa $XSA_FILE_NAME`.bit

### Create create_fsbl_project.tcl file used by xsct to create the fsbl.
echo 'hsi open_hw_design '$XSA_FILE_NAME > $BUILD_DIR/create_fsbl_project.tcl
echo 'set cpu_name [lindex [hsi get_cells -filter {IP_TYPE==PROCESSOR}] 0]' >> $BUILD_DIR/create_fsbl_project.tcl
echo 'platform create -name hw0 -hw '$XSA_FILE_NAME' -os standalone -out ./build/sdk -proc $cpu_name' >> $BUILD_DIR/create_fsbl_project.tcl

#echo 'exec sed -i "36i #define FSBL_DEBUG_INFO" ./build/sdk/hw0/zynq_fsbl/fsbl_debug.h' >> $BUILD_DIR/create_fsbl_project.tcl


echo 'exec ../patch.sh' >> $BUILD_DIR/create_fsbl_project.tcl
...
SYSTEM_TOP_BIT_PATH="$BUILD_DIR/build/sdk/hw0/hw/$BIT_FILE_NAME"
...
echo $BIT_FILE_NAME >> $OUTPUT_DIR/zynq.bif
...
cp $SYSTEM_TOP_BIT_PATH $OUTPUT_DIR/$BIT_FILE_NAME
